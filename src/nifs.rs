...
pub struct R1CS<F: PrimeField> {
    pub A: Vec<Vec<F>>, // Matrix
    pub B: Vec<Vec<F>>, // Matrix
    pub C: Vec<Vec<F>>, // Matrix
}
    ...
    <C as CurveGroup>::BaseField: Absorb,
{
    ...
    pub fn verify(
        ...
        let r2 = r * r;
        // check if cmE = cmE_1 + cmT * r + cmE_2 * r^2
    ...
    pub fn verify_commitments(
        tr: &mut Transcript<C::ScalarField, C>,
        pedersen_perams: &PedersenParams<C>,
        phi: Phi<C>,
        cmT: Commitment<C>,
        ...
        let cmE_verified = Pedersen::verify(pedersen_perams, tr, phi.cmE, cmE_proof);
        let cmW_verified = Pedersen::verify(pedersen_perams, tr, phi.cmW, cmW_proof);
        let cmT_verified = Pedersen::verify(pedersen_perams, tr, cmT, cmT_proof);
        cmE_verified && cmW_verified && cmT_verified
    }
}
